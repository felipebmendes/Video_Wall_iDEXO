var SlideShow = SuperWidget.extend({
	...
	definePreferences: function () {
		...
		this.interval = $('#interval' + mode).val();
	},
	...
	chooseDirectory: function () {
		...
		ECM.findDocument.panel.bind('panel-button-0', function () {
			if (ECM.findDocument.dataTable.selectedRows.length === 0) {
				WCMC.messageWarn('${i18n.getTranslation("kit_slideshow.edit.error.atleastone")}' + '.');
				return;
			}
			...
			if (ECM.findDocument.dataTable.selectedRows.length === 1) {
				...
				WCMC.messageWarn('${i18n.getTranslation("kit_slideshow.edit.error.justone")}' + '.');
				return;
...
var FLUIGC = function(a, b, c) {
    var d = {};
    return a.carousel = function(a, e) {
        function f(a, c) {
            c || (c = {}),
            c.target = a;
            var d = {
                id: "fluigCarousel_" + Math.ceil(1e3 * Math.random()),
                images: [],
                indicators: !0,
                startIndex: 0,
                interval: 5e3,
            ...
            return r = Object.create(d),
            b.extend(r, c),
            r
        }
        function g() {
            if (!c)
                throw new Error("Carousel component: FLUIGCTemplates not found.");
            h("template_carousel_basic")
        }
        function h(a) {
            d[a] = b(c["carousel-basic"]).html(),
            i()
        }
        function i() {
            1 == r.autoSize && (q = "100%"),
            r.startIndex < r.images.length ? r.images[r.startIndex].startIndex = !0 : (console && console.warn("Fluig Carousel: startIndex is greater than the images array length. startIndex will be set to 0."),
            r.startIndex = 0,
            r.images[r.startIndex].startIndex = !0);
            for (var a = 0; a < r.images.length; a++)
                r.images[a].index = a;
            if (r.template && r.template.length)
                for (var a = 0; a < r.images.length; a++) {
                    var c = b(r.template).html().trim();
                    r.images[a].template = Mustache.render(c, r.images[a])
                }
            var e = {
                id: r.id,
              ...
              , f = Mustache.render(d.template_carousel_basic, e);
            b(r.target).append(f),
            j()
        }
        function j() {
            b("#" + r.id).on("slide.bs.carousel", function() {
                var a = b.Event("fluig.carousel.slide");
                b("#" + r.id).trigger(a)
            }),
            r.resize && b("#" + r.id).removeAttr("style"),
            b(r.target).find("[data-img-carousel-link]").on("click", function() {
                var a = b(this).data("img-carousel-link")
                  , c = b(this).data("img-carousel-target") || "_blank";
                if (FLUIGC.utilities.checkBrowser().isIe() || FLUIGC.utilities.checkBrowser().isFirefox()) {
                    window.open(a, c).location
                } else {
                    b("<a>", {
                        href: a || "#",
                        target: c
        ...
        function k(a, c) {
            b(r.target).on(a, function(a) {
                c(a)
            })
        }
        function l() {
            b("#" + r.id).carousel("cycle")
        }
        function m() {
            b("#" + r.id).carousel("pause")
        }
        function n() {
            b("#" + r.id).carousel("prev")
        }
        function o() {
            b("#" + r.id).carousel("next")
        }
        function p(a) {
            b("#" + r.id).carousel(a)
        }
        var q, r = f(a, e);
        return r.autoSize = JSON.parse(r.autoSize),
        r.resize = JSON.parse(r.resize),
        g(),
        {
            play: l,
            pause: m,
